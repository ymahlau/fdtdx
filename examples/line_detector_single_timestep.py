...
def main():
    exp_logger = Logger(
        experiment_name="1d_line_detector_visualization_test",
    )
    ...
    # Setup boundaries
    bound_cfg = BoundaryConfig.from_uniform_bound(thickness=30, boundary_type="pml")
    ...
    # Create three different 1D line detectors along each axis
    # Each will record energy at a single time step in the middle of the simulation
    line_detector_x = EnergyDetector(
        name="1D Line X",
        partial_grid_shape=(None, 1, 1),  # Line along X-axis
        partial_real_shape=(None, 0.1e-6, 0.1e-6),
        time_steps=[config.time_steps_total // 2],  # Single time step
    ...
    # Place all three line detectors to cross at the center of the volume
    constraints.extend(
        [
    ...
    # Generate and save detector visualizations - this will test our 1D detector plotting
    ...
    logger.info("Complete! Check the generated plots to confirm 1D visualization is working correctly.")
