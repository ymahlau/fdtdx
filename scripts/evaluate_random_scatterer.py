...
def main(
    ...
    placement_constraints.extend([
        *energy_last_step.same_position_and_size(volume)
    ])
    ...
    placement_constraints.extend([
        *video_detector.same_position_and_size(volume)
    ])
    ...
    def plot_field_component_planes(
        field_component, component: str, design: jax.Array, norm=None
    ):
        ...
        for ax, (plane_name, plane_data) in zip(axs, planes.items()):
            ...
            ax.contour(
                design_plane, levels=[0.5], colors="grey", linestyles="-", linewidths=3
            )
            fig.colorbar(im, ax=ax)
            ax.set_title(f"{component} component in {plane_name} plane")
            ax.set_xlabel(
                "x" if "x" in plane_name else ("y" if "y" in plane_name else "z")
            )
            ax.set_ylabel(
                "y" if "xy" in plane_name else ("z" if "yz" in plane_name else "x")
            )
        return fig

    def plot_absolute_error_planes(ground_truth, prediction, component, design):
        error = jnp.abs(ground_truth - prediction)
        return plot_field_component_planes(
            error, component, design=design, norm="log"
        ), error.mean()
    ...
    def evaluate(epoch, key):
        ...
        for i, component in enumerate(components):
            ...
            exp_logger.savefig(
                ...
                plot_field_component_planes(
                    normalized_ground_truth, component=component, design=design
                ),
            )
            exp_logger.savefig(
                exp_logger.cwd,
                ...
                plot_field_component_planes(
                    normalized_prediction, component=component, design=design
                ),
            )
    ...
    epochs = 2
    evaluation_task_id = exp_logger.progress.add_task(
        "Evaluation", start=True, total=epochs
    )
