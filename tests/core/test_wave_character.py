...
def test_wavecharacter_consistency_across_all_conversions():
    ...
    assert jnp.allclose(wave_from_freq.get_frequency().value(), wave_from_period.get_frequency().value(), rtol=1e-12)
    assert jnp.allclose(
        wave_from_freq.get_frequency().value(), wave_from_wavelength.get_frequency().value(), rtol=1e-12
    )

    # All should have the same period
    assert jnp.allclose(wave_from_freq.get_period().value(), wave_from_period.get_period().value(), rtol=1e-12)
    assert jnp.allclose(wave_from_freq.get_period().value(), wave_from_wavelength.get_period().value(), rtol=1e-12)

    # All should have the same wavelength
    assert jnp.allclose(wave_from_freq.get_wavelength().value(), wave_from_period.get_wavelength().value(), rtol=1e-12)
    assert jnp.allclose(
        wave_from_freq.get_wavelength().value(), wave_from_wavelength.get_wavelength().value(), rtol=1e-12
    )
