...
def test_expand_to_3x3_value():
    ...
    assert test_none is None
    assert (test_scalar == jnp.array([
        [[[[1.0]]], [[[zero]]], [[[zero]]]],
        [[[[zero]]], [[[1.0]]], [[[zero]]]],
        [[[[zero]]], [[[zero]]], [[[1.0]]]],
    ])).all()
    assert (test_isotropic == jnp.stack([
        jnp.stack([iso_a, zeros, zeros]), 
        jnp.stack([zeros, iso_a, zeros]), 
        jnp.stack([zeros, zeros, iso_a])
    ])).all()
    assert (test_diagonal == jnp.stack([
        jnp.stack([aniso_a, zeros, zeros]), 
        jnp.stack([zeros, aniso_b, zeros]), 
        jnp.stack([zeros, zeros, aniso_c])
    ])).all()
