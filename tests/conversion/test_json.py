...
def test_energy_detector_json():
    det = EnergyDetector(
        partial_real_shape=(1e-6 * m, None, None),
        name="Detector",
    )
    d = export_json(det)
    ...
    assert rec.partial_real_shape == (1e-6 * m, None, None)


def test_object_container_json():
    obj_list = [
        EnergyDetector(),
        UniformPlaneSource(wave_character=WaveCharacter(wavelength=1e-6 * m), direction="-"),
        SimulationVolume(),
    ]
    ...
    assert rec.object_list[1].wave_character.wavelength == 1e-6 * m
