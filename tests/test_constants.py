...
from fdtdx.constants import c, eps0, eta0, mu0, wavelength_to_period
...
def test_wavelength_to_period_unit_consistency():
    ...
    assert period.unit.dim == expected_period.unit.dim, "Period units should match manual calculation"

    # Verify the values are identical
    assert abs(period.value() - expected_period.value()) < 1e-15, "Period value should match manual calculation"
