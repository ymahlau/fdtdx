...
def test_construction_stores_spacings():
...
def test_resolve_returns_rectilinear_grid():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
    assert isinstance(r, RectilinearGrid)


def test_resolve_shape_matches_request():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
    assert r.shape == (4, 6, 8)


def test_resolve_x_edges_centered_at_origin():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
...
def test_resolve_y_edges_centered_at_origin():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
...
def test_resolve_z_edges_centered_at_origin():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
...
def test_resolve_x_spacing_is_uniform():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=40e-9)
    r = g.resolve((4, 6, 8))
...
def test_resolve_axes_independent():
...
def test_resolve_with_nonzero_center():
...
def test_resolve_odd_x_raises():
    g = QuasiUniformGrid(dx=10e-9, dy=10e-9, dz=10e-9)
    with pytest.raises(ValueError, match="Axis 0"):
        g.resolve((3, 4, 4))


def test_resolve_odd_y_raises():
    g = QuasiUniformGrid(dx=10e-9, dy=10e-9, dz=10e-9)
    with pytest.raises(ValueError, match="Axis 1"):
        g.resolve((4, 5, 4))


def test_resolve_odd_z_raises():
    g = QuasiUniformGrid(dx=10e-9, dy=10e-9, dz=10e-9)
    with pytest.raises(ValueError, match="Axis 2"):
        g.resolve((4, 4, 7))


def test_resolve_all_even_does_not_raise():
    g = QuasiUniformGrid(dx=10e-9, dy=10e-9, dz=10e-9)
...
def test_coord_to_index_at_center_is_zero():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
...
def test_cell_volume_shape_and_value():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
...
def test_face_area_normal_to_z():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
...
def test_face_area_normal_to_x():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
...
def test_config_time_step_uses_min_spacing():
    import math
...
def test_config_uniform_spacing_raises_when_anisotropic():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=10e-9)
    config = SimulationConfig(grid=g, time=10e-15)
...
def test_config_uniform_spacing_succeeds_when_isotropic():
    g = QuasiUniformGrid(dx=10e-9, dy=10e-9, dz=10e-9)
    config = SimulationConfig(grid=g, time=10e-15)
    assert config.uniform_spacing() == pytest.approx(10e-9)


def test_config_resolved_grid_none_before_placement():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
    config = SimulationConfig(grid=g, time=10e-15)
    assert config.resolved_grid is None


def test_config_resolve_grid_returns_rectilinear():
    g = QuasiUniformGrid(dx=10e-9, dy=20e-9, dz=30e-9)
    config = SimulationConfig(grid=g, time=10e-15)
...
def test_placement_resolves_grid(anisotropic_config, material):
...
def test_placement_object_sizes_in_cells(anisotropic_config, material):
...
def test_placement_object_sizes_xy(anisotropic_config, material):
...
def test_placement_centered_domain(anisotropic_config, material):
...
def test_placement_grid_coordinate_constraint(isotropic_quasi_config, material):
    ...
    slices, errors = resolve_object_constraints(objects=[volume, obj], constraints=[c], config=isotropic_quasi_config)
    assert errors["obj"] is None
    assert slices["obj"][0] == (6, 10)
    assert slices["obj"][2] == (3, 7)


def test_isotropic_quasi_uniform_matches_uniform_grid():
