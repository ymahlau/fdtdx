...
class TestCondFn:
    def test_returns_true_when_above_start(self):
        assert cond_fn((10, None), start_time_step=5) is True

    def test_returns_false_at_start(self):
        assert cond_fn((5, None), start_time_step=5) is False

    def test_returns_false_below_start(self):
        assert cond_fn((3, None), start_time_step=5) is False
...
class TestBackward:
    ...
    def test_periodic_boundary_positive_direction(self, mock_arrays, mock_objects, key, patched_updates):
        ...
        assert "H" in mock_arrays._aset_log

    def test_periodic_boundary_negative_direction(self, mock_arrays, mock_objects, key, patched_updates):
        boundary = Mock(spec=PeriodicBoundary)
        boundary.axis = 1
        boundary.direction = "-"
