...
class PlaneSource(DirectionalPlaneSourceBase, ABC):
    ...
    def _get_center(
        ...
        center = jnp.asarray(
            [
                center_horizontal + horizontal_offset,
                center_vertical + vertical_offset
            ],
            dtype=jnp.float32,
        ).squeeze()
        return center
    ...
    def update_E(
        ...
        if (
            self._E is None or self._H is None 
            or self._time_offset_E is None 
            or self._time_offset_H is None
        ):
            raise Exception(f"Need to apply random key before calling update")
        time_step = linear_interpolated_indexing(
            time_step.reshape(1,), 
            self._time_step_to_on_idx,
        )
        
        inv_perm_slice = inv_permittivities[*self.grid_slice]
        
        delta_t = self._config.time_step_duration
        
        # vertical incident wave part
        time_phase_H_v = (
            2 * jnp.pi * (time_step - 0.5 + self._time_offset_H[self.vertical_axis]) * delta_t 
            / self.period
            + self.phase_shift
        )
        H_v_inc = self._H[self.vertical_axis] * jnp.cos(time_phase_H_v)
        H_v_inc = H_v_inc * self._config.courant_factor * inv_perm_slice
        H_v_inc = jax.lax.stop_gradient(H_v_inc)
        
        # horizontal incident wave part
        time_phase_H_h = (
            2 * jnp.pi * (time_step - 0.5 + self._time_offset_H[self.horizontal_axis]) * delta_t 
            / self.period
            + self.phase_shift
        )
        H_h_inc = self._H[self.horizontal_axis] * jnp.cos(time_phase_H_h)
        H_h_inc = H_h_inc * self._config.courant_factor * inv_perm_slice
    ...
    def update_H(
        ...
        inv_perm_slice = inv_permeabilities[*self.grid_slice]
        ...
        E_h_inc = E_h_inc * self._config.courant_factor * inv_perm_slice
        ...
        E_v_inc = E_v_inc * self._config.courant_factor * inv_perm_slice
...
class LinearlyPolarizedPlaneSource(PlaneSource, ABC):
    ...
    def _get_raw_EH_polarization(
        ...
        if e_pol is None:
            if h_pol is None:
                raise Exception(f"Need to specify either E or H polarization")
    ...
    def get_EH_variation(
        ...
        jax.Array,  # E: (*grid_shape, 3)
        jax.Array,  # H: (*grid_shape, 3)
        jax.Array,  # phase_E: (3, *grid_shape)
        jax.Array,  # phase_H: (3, *grid_shape)
    ]:  
...
class GaussianPlaneSource(LinearlyPolarizedPlaneSource):
    ...
    def _gauss_profile(
        ...
        grid = (
            jnp.stack(
                jnp.meshgrid(*map(jnp.arange, (height, width)), indexing="xy"), axis=-1
            )
            - jnp.asarray(center)
        ) / jnp.asarray(radii)
...
class HardConstantAmplitudePlanceSource(DirectionalPlaneSourceBase):
    ...
    def update_E(
        ...
        delta_t = self._config.time_step_duration
        time_phase = (
            2 * jnp.pi * time_step * delta_t / self.period
            + self.phase_shift
        )
        magnitude = jnp.real(
            self.amplitude * jnp.exp(-1j * time_phase)
        )
