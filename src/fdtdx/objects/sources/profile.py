...
from drinx import DataClass, static_field, field
import numpy as np
...
class CustomTimeSignalProfile(TemporalProfile):
    ...
    time_step_duration: float = static_field()

    #: Simulation time at which ``signal[0]`` was sampled (seconds).
    start_time: float = static_field(default=0.0)

    #: Interpolation mode: ``"linear"`` (default) or ``"nearest"``.
    interpolation: Literal["linear", "nearest"] = static_field(default="linear")

    #: Value returned for times outside the sampled window.
    outside_value: float = _field(default=0.0)
