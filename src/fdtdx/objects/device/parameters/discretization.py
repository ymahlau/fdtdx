...
class ClosestIndex(ParameterTransformation):
    ...
    mapping_from_inverse_permittivities: bool = static_field(default=False)
...
class PillarDiscretization(ParameterTransformation):
    ...
    distance_metric: Literal["euclidean", "permittivity_differences_plus_average_permittivity"] = static_field(
    ...
    _allowed_indices: jax.Array = static_private_field(default=None)
    _check_single_array: bool = static_private_field(default=True)
    _fixed_input_type: ParameterType | Sequence[ParameterType] | None = static_private_field(
        default=ParameterType.CONTINUOUS
    )
